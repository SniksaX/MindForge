...
const PricingList = () => {
  ...
  const handleButtonClick = (item) => {
    if (item.id === "0") {
      ...
      window.location.href = "mailto:siraj.rahal.dev@gmail.com"; // For email, leave as is
    }
  };
    ...
    <div className="flex gap-[1rem] max-lg:flex-wrap">
      {pricing.map((item) => (
        <div key={item.id} className="w-[19rem] max-lg:w-full h-full px-6 bg-n-8 border border-n-6 rounded-[2rem] lg:w-auto even:py-14 odd:py-8 odd:my-4 [&>h4]:first:text-color-2 [&>h4]:even:text-color-1 [&>h4]:last:text-color-3">
          ...
          <Button
            onClick={() => handleButtonClick(item)} // Use onClick instead of href for navigation
            className="w-full mb-6"
            white={!!item.price}
          >
            {item.id === "0" ? "Book appointment" : item.price ? "Get started" : "Contact us"}
          </Button>
          <ul>
